...
MAX_ITER="${3:-10}"
INTERVAL="${4:-15}"
AI_PROVIDER="${CLANKER_AI_PROVIDER:-claude}"
AI_MODEL="${CLANKER_AI_MODEL:-}"
DIRTY_WORKTREE_POLICY="${CLANKER_DIRTY_WORKTREE_POLICY:-abort}"
ONE_TIME_CLEAN_MODE="${CLANKER_ONE_TIME_CLEAN_MODE:-false}"
HISTORY_CLEAN_MODE="${CLANKER_HISTORY_CLEAN_MODE:-cleanup_commit}"
HISTORY_SCOPE_CLASSIFIER="${CLANKER_HISTORY_SCOPE_CLASSIFIER:-ai}"
SKIP_CI_FIX="${CLANKER_SKIP_CI_FIX:-false}"
QUICK_RETRY_INTERVAL=2
RUN_CODEX_LAST_RESULT="unknown" # changed | no_changes | blocked | unknown
...
PR_MERGED_AT=""
BASE_REPO_DIR=""
MONITOR_WORKTREE_DIR=""
MONITOR_WORKTREE_BRANCH=""
CLEANUP_DONE=0

# Normalize provider and fall back safely
AI_PROVIDER="$(echo "$AI_PROVIDER" | tr '[:upper:]' '[:lower:]' | xargs)"
if [ "$AI_PROVIDER" != "claude" ] && [ "$AI_PROVIDER" != "codex" ]; then
  AI_PROVIDER="claude"
fi

DIRTY_WORKTREE_POLICY="$(echo "$DIRTY_WORKTREE_POLICY" | tr '[:upper:]' '[:lower:]' | xargs)"
if [ "$DIRTY_WORKTREE_POLICY" != "stash" ] && [ "$DIRTY_WORKTREE_POLICY" != "abort" ]; then
  DIRTY_WORKTREE_POLICY="abort"
fi

ONE_TIME_CLEAN_MODE="$(echo "$ONE_TIME_CLEAN_MODE" | tr '[:upper:]' '[:lower:]' | xargs)"
if [ "$ONE_TIME_CLEAN_MODE" != "true" ] && [ "$ONE_TIME_CLEAN_MODE" != "false" ]; then
  ONE_TIME_CLEAN_MODE="false"
fi

HISTORY_CLEAN_MODE="$(echo "$HISTORY_CLEAN_MODE" | tr '[:upper:]' '[:lower:]' | xargs)"
if [ "$HISTORY_CLEAN_MODE" != "cleanup_commit" ]; then
  echo -e "${YELLOW}⚠️ Unsupported history clean mode '${HISTORY_CLEAN_MODE}'. Falling back to cleanup_commit.${RESET}"
  HISTORY_CLEAN_MODE="cleanup_commit"
fi

HISTORY_SCOPE_CLASSIFIER="$(echo "$HISTORY_SCOPE_CLASSIFIER" | tr '[:upper:]' '[:lower:]' | xargs)"
if [ "$HISTORY_SCOPE_CLASSIFIER" != "ai" ] && [ "$HISTORY_SCOPE_CLASSIFIER" != "heuristic" ]; then
  echo -e "${YELLOW}⚠️ Unsupported history scope classifier '${HISTORY_SCOPE_CLASSIFIER}'. Falling back to ai.${RESET}"
  HISTORY_SCOPE_CLASSIFIER="ai"
fi
...
echo -e "${CYAN}│${RESET}  ${DIM}Base repo dir:${RESET} ${BASE_REPO_DIR:-unknown}"
echo -e "${CYAN}│${RESET}  ${DIM}PR head branch:${RESET} ${EXPECTED_PR_HEAD_BRANCH:-unknown}"
echo -e "${CYAN}│${RESET}  ${DIM}AI:${RESET} $AI_PROVIDER ${AI_MODEL:+($AI_MODEL)}"
echo -e "${CYAN}│${RESET}  ${DIM}Dirty-worktree policy:${RESET} $DIRTY_WORKTREE_POLICY"
echo -e "${CYAN}│${RESET}  ${DIM}One-time clean mode:${RESET} $ONE_TIME_CLEAN_MODE"
echo -e "${CYAN}│${RESET}  ${DIM}History clean strategy:${RESET} $HISTORY_CLEAN_MODE (non-rewrite)"
echo -e "${CYAN}│${RESET}  ${DIM}History scope classifier:${RESET} $HISTORY_SCOPE_CLASSIFIER"
echo -e "${CYAN}│${RESET}  ${DIM}Skip CI fix:${RESET} $SKIP_CI_FIX"
...
# Function to run review-comment handling flow with selected provider
run_handle_comments() {
  local pre_comment_ids
  pre_comment_ids=$(get_unresolved_thread_ids)

  if [ "$AI_PROVIDER" = "codex" ]; then
    run_codex_prompt "Handle review comments for PR #$PR_NUM in $REPO.
Use unresolved thread data from this file ONLY: $THREADS_FILE

Hard constraints:
1) Work only in this checkout: $REPO_DIR
2) Treat the thread file as the source of truth for scope. Do NOT inspect, list, or reference any other PR.
3) Allowed GitHub actions are limited to this PR (#$PR_NUM), primarily resolving SKIP/FIX threads when appropriate.
4) Apply only focused fixes for unresolved threads from the file.
5) If code changes are needed, commit and push to PR head branch: $EXPECTED_PR_HEAD_BRANCH
6) Do NOT leave tracked uncommitted changes.
7) At the very end, print exactly one status line:
   - @@RUN_RESULT:CHANGED@@
   - @@RUN_RESULT:NO_CHANGES_NEEDED@@
   - @@RUN_RESULT:BLOCKED:<reason>@@
8) If any original unresolved threads cannot be resolved this run, use BLOCKED (not NO_CHANGES)." "comment handling" "true"
  else
    local before_head
    before_head=$(git -C "$REPO_DIR" rev-parse HEAD 2>/dev/null || true)

    run_claude_prompt "/handle-pr-comments $PR_NUM $REPO --threads-file $THREADS_FILE" "comment handling" || return 1

    local after_head
    after_head=$(git -C "$REPO_DIR" rev-parse HEAD 2>/dev/null || true)

    if [ "$after_head" != "$before_head" ]; then
      echo -e "${DIM}🚀 Pushing commit(s) to PR head branch (${EXPECTED_PR_HEAD_BRANCH}) for comment handling...${RESET}"
      if ! push_head_to_pr_branch "comment handling"; then
        return 1
      fi
    fi
    ...
    dirty_after=$(git -C "$REPO_DIR" status --porcelain --untracked-files=no)
    if [ -n "$dirty_after" ]; then
      echo -e "${RED}❌ Tracked uncommitted changes remain after comment handling.${RESET}"
      return 1
    fi
  fi

  if ! resolve_review_threads_from_csv "$pre_comment_ids"; then
    return 1
  fi
}

if [ "$ONE_TIME_CLEAN_MODE" = "true" ]; then
  run_one_time_history_clean_mode
fi
...
for iter in $(seq 1 $MAX_ITER); do
  ...
  # ═══════════════════════════════════════════════════════════════
  # STEP 1: Check merge status and fix conflicts if needed
  # ═══════════════════════════════════════════════════════════════
  echo ""
  echo -e "${DIM}🧩 Checking merge status...${RESET}"
  merge_status=$(check_merge_status)
  case "$merge_status" in
    clean) merge_color="${GREEN}" ;;
    conflicting) merge_color="${RED}" ;;
    unknown) merge_color="${YELLOW}" ;;
    *) merge_color="${DIM}" ;;
  esac
  echo -e "🔀 Merge Status: ${merge_color}${merge_status}${RESET}"
  echo "@@MERGE_STATUS:$merge_status@@"

  if [ "$merge_status" = "conflicting" ]; then
    echo -e "${RED}❌ Merge conflicts detected - fixing first...${RESET}"
    if ! run_fix_merge; then
      iteration_failed=1
      echo "@@AGENT_ERROR:merge_fix@@"
      echo -e "${YELLOW}⚠️ Merge fix flow failed this iteration.${RESET}"
    fi
    merge_status=$(check_merge_status)
    echo "@@MERGE_STATUS:$merge_status@@"
  elif [ "$merge_status" = "unknown" ]; then
    echo -e "${YELLOW}⚠️ Merge status unknown; continuing with CI and comments.${RESET}"
  else
    echo -e "${GREEN}✅ No merge conflicts${RESET}"
  fi

  # ═══════════════════════════════════════════════════════════════
  # STEP 2: Check CI status (wait if pending, max 3x5min = 15min)
  # ═══════════════════════════════════════════════════════════════
  echo ""
  ...
  # except immediately after a merge-fix commit where pending is expected.
  if [ "$ci_status" = "pending" ] && [ "${MERGE_FIX_CREATED_COMMIT:-0}" -eq 1 ]; then
    CI_PENDING_WAIT_SKIPPED=1
    echo -e "${YELLOW}⏭️ CI is pending after merge-fix push; skipping wait this iteration.${RESET}"
    echo "@@CI_WAIT_SKIPPED:post_merge_fix@@"
  else
    ci_waits=0
    max_ci_waits=1
  ...
  # ═══════════════════════════════════════════════════════════════
  # STEP 3: Fix CI if failed (unless SKIP_CI_FIX is enabled)
  # ═══════════════════════════════════════════════════════════════
  if [ "$ci_status" = "failure" ]; then
    if [ "$SKIP_CI_FIX" = "true" ]; then
      echo ""
      echo -e "${YELLOW}⏭️ CI failing but SKIP_CI_FIX is enabled - skipping fix${RESET}"
    else
      echo ""
      echo -e "${RED}❌ CI failing - fixing first...${RESET}"
      if ! run_fix_ci; then
        iteration_failed=1
        echo "@@AGENT_ERROR:ci_fix@@"
        echo -e "${YELLOW}⚠️ CI fix flow failed this iteration.${RESET}"
      fi
    fi
  elif [ "$ci_status" = "success" ]; then
    echo -e "${GREEN}✅ CI passing${RESET}"
  elif [ "$ci_status" = "pending" ]; then
    if [ "${CI_PENDING_WAIT_SKIPPED:-0}" -eq 1 ]; then
      echo -e "${YELLOW}⏭️ CI pending from recent merge-fix push; continuing without waiting.${RESET}"
    else
  ...
  # ═══════════════════════════════════════════════════════════════
  # STEP 4: Fetch and check for unresolved PR comments
  # ═══════════════════════════════════════════════════════════════
  echo ""
  ...
  if ! current_count=$(fetch_threads); then
    iteration_failed=1
    echo "@@AGENT_ERROR:threads_fetch@@"
    echo -e "${YELLOW}⚠️ Failed to fetch unresolved thread data; skipping comment handling this iteration.${RESET}"
  fi
  ...
  # Check if PR is clean (no comments, CI passing, and no merge conflicts)
  if [ "$current_count" -eq 0 ] && [ "$ci_status" = "success" ] && [ "$merge_status" = "clean" ]; then
    echo "@@STATUS:clean@@"
    echo -e "${GREEN}${BOLD}✅ PR is clean!${RESET} ${GREEN}No unresolved comments, CI passing, and mergeable.${RESET}"
    echo ""
    echo -e "${GREEN}╭─────────────────────────────────────────────────────────────╮${RESET}"
    echo -e "${GREEN}│${RESET}  ${GREEN}${BOLD}✅ Monitor Complete${RESET} - PR ${MAGENTA}#$PR_NUM${RESET}"
  ...
  if [ "$current_count" -gt 0 ]; then
    ...
    if ! run_handle_comments; then
      ...
      if ! post_comment_count=$(fetch_threads); then
        ...
        if [ -n "$current_ids" ]; then
          for id in $(echo "$current_ids" | tr ',' '\n'); do
            if [ -n "$id" ] && ! echo ",$post_comment_ids," | grep -q ",$id,"; then
              resolved_existing=1
              break
            fi
          done
        ...
        if [ "$post_comment_count" -lt "$current_count" ] || [ "$resolved_existing" -eq 1 ]; then
          echo -e "${GREEN}✅ Comment handling reduced unresolved threads (${current_count} → ${post_comment_count}).${RESET}"
          update_state "$iter" "$post_comment_ids"
        else
          iteration_failed=1
          echo "@@AGENT_ERROR:comment_not_resolved@@"
          if [ "$AI_PROVIDER" = "codex" ] && [ "${RUN_CODEX_LAST_RESULT:-unknown}" = "no_changes" ]; then
            echo -e "${YELLOW}⚠️ Codex reported no changes, but unresolved threads did not decrease.${RESET}"
          else
            echo -e "${YELLOW}⚠️ Comment handling completed but unresolved threads did not decrease.${RESET}"
          fi
        fi
      fi
    fi
    ...
    echo -e "${YELLOW}⚠️ Skipping comment workflow because unresolved-thread state is unknown this iteration.${RESET}"
  fi
